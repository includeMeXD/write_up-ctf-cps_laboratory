...
Analisis: Memfilter trafik FTP (ftp atau tcp.stream eq <stream_id>).
...
ftp 34.142.169.86
# Login dengan apanihftp : akucintacps
...
Berdasarkan hint, terdapat kerentanan SQL Injection pada parameter id di /product.php. Eksploitasi dilakukan menggunakan SQLMap untuk mengekstrak data sensitif dari database.

Ekstraksi Database
Menggunakan SQLMap untuk mengidentifikasi database.
...
sqlmap -u "http://34142.169.86/product.php?id=1" -D ctf_db -T users -C username,ssh_key --dump
Hasil:
...
vim dapat digunakan untuk memanggil shell sistem. Karena dijalankan dengan sudo, shell yang terpanggil akan memiliki akses root.
